...
def getCategoryFrequenceMax(df_data, df_feature, group, fea='', name=''):
    df_temp = df_feature.groupby(group)[fea].apply(
        lambda series: series.value_counts(dropna=False).index[0]).reset_index()
    df_temp.rename(columns={fea: name}, inplace=True)
    df_data = df_data.merge(df_temp, on=group, how='left')
    return df_data
...
def getCategoryFrequenceMaxRatio(df_data, df_feature, group, fea='', name=''):
    df_temp = df_feature.groupby(group)[fea].apply(
        lambda series: list(series.value_counts(dropna=False).values)[0] / series.shape[0]).reset_index()
    df_temp.rename(columns={fea: name}, inplace=True)
    df_data = df_data.merge(df_temp, on=group, how='left')
    return df_data
